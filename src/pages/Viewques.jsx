...
const Viewques = (props) => {
  ...
  return (
    <div className="flex flex-col  bg-slate-50  border border-gray-200 shadow-lg mx-0  py-8 px-4 md:p-8 rounded-3xl  mt-4 w-full md:mx-0 md:w-[80%] ">
      {props.questions > 0 ? (<><div className="flex flex-col md:flex-row items-center">
        <div
          className="hidden md:block hover:bg-gray-400 rounded-full"
        ...
        <div className="grow md:mx-6 self-center w-[90%]">
            ...
            {props.questions[currentQuestion]?.imageDescription && (
              <div className="flex justify-center">
                <img
                  ...
                  src={props.questions[currentQuestion]?.imageDescription.url}
                  alt=""
                />
              </div>
            )}
          ...
          {props.questions[currentQuestion].options.map((option, index) => (
            <label
              className={"block text-gray-800 font-bold border-[3px] text-base md:text-lg rounded-lg md:py-3 md:px-3 px-2 py-2 my-4"}
              key={index}
            >
              {option.text}
              {option?.image && (
        ...
        <div className="flex flex-row justify-between w-full md:hidden">
          <div
            className="visible md:hidden hover:bg-gray-400 rounded-full"
            onClick={handlePrevQuestion}
          >
        ...
        <div className="flex justify-evenly md:justify-between mt-8 md:mx-6       md:gap-0">
          ...
          <p className="text-[crimson] font-medium text-3xl text-center">No Question Found!</p>
          <p className="text-[crimson] font-medium text-xl text-center">Add Questions to view</p>
        </>)}
